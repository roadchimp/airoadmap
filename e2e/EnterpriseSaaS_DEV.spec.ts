...
test.describe('Enterprise SaaS Inc. (ESI) AI Transformation Assessment', () => {
  ...
  test.beforeEach(async ({ page, context }) => {
    ...
    // Check if running on localhost/127.0.0.1 - skip auth for local development
    const isLocalhost = APP_URL.includes('localhost') || APP_URL.includes('127.0.0.1');
    
    if (isLocalhost) {
      console.log("Running on localhost - handling local authentication");
      
      // Navigate to dashboard first
      await page.goto(`${APP_URL}/dashboard`);
      await page.waitForLoadState('networkidle');
      
      // Check if we need to sign in (redirected to login page)
      ...
      if (currentUrl.includes('/login')) {
        ...
        // Check if we need to sign in from dashboard
      ...
      await expect(page.getByRole('heading', { name: /AI Transformation Assessment/i })).toBeVisible({ timeout: 10000 });
      console.log("Dashboard loaded successfully on localhost");
      return;
    }
    
    // Production authentication flow (existing code)
    await page.waitForLoadState('networkidle');

    // Check if we're on Vercel's login page or the app's login page
    const isVercelLogin = page.url().includes('vercel.com/login');
    const isAppLogin = page.url().includes(`${APP_URL}/login`) || page.url() === `${APP_URL}/` || page.url() === APP_URL;
    ...
    await test.step('Initiate and complete assessment wizard', async () => {
      ...
      while (!reportGenerated && attempts < maxAttempts) {
        ...
        if (!reportGenerated && attempts < maxAttempts) {
          ...
          await page.waitForTimeout(15000); // Wait 5 seconds before retrying
        }
      }
