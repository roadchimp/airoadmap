...
/**
 * Script to migrate ai_capabilities table
 */
...
async function main() {
  try {
    console.log("Starting AI capabilities migration...");
    
    // Get count before migration
    const beforeCount = await countRows(destPool, 'ai_capabilities');
    console.log(`Current row count in ai_capabilities: ${beforeCount}`);
    
    // Clear the table first
    await clearTable(destPool, 'ai_capabilities');
    
    // Get data from source
    const aiCapabilitiesData = await getTableData(sourcePool, 'ai_capabilities');
    
    // Insert data into destination
    await insertData(destPool, 'ai_capabilities', aiCapabilitiesData);
    
    // Reset sequence if needed
    await resetSequence(destPool, 'ai_capabilities', 'id');
    
    // Get count after migration
    const afterCount = await countRows(destPool, 'ai_capabilities');
    console.log(`Row count in ai_capabilities after migration: ${afterCount}`);
    
    console.log("AI capabilities migration completed");
  } catch (error) {
