...
/**
 * Script to migrate capability_job_roles table
 * This script assumes ai_capabilities and job_roles have been migrated
 */
  ...
  for (const row of rows) {
    ...
    try {
      ...
      const query = `
        ...
        VALUES (${placeholders})
        ON CONFLICT (capability_id, job_role_id) DO NOTHING
      `;
      ...
      console.error(`Error inserting row (capability_id: ${row.capability_id}, job_role_id: ${row.job_role_id}):`, error.message);
      ...
      if (error.message.includes('foreign key')) {
        ...
        try {
          ...
          const jobRoleResult = await errorClient.query(
            'SELECT EXISTS(SELECT 1 FROM job_roles WHERE id = $1) as exists',
            [row.job_role_id]
          );
          
          console.error(`Capability ${row.capability_id} exists: ${capabilityResult.rows[0].exists}`);
          console.error(`Job role ${row.job_role_id} exists: ${jobRoleResult.rows[0].exists}`);
        } catch (checkError) {
...
async function verifyDependencies() {
  // Verify that capabilities and job roles have been migrated
  const capabilitiesClient = await destPool.connect();
  const jobRolesClient = await destPool.connect();
  ...
  try {
    ...
    const jobRolesResult = await jobRolesClient.query('SELECT COUNT(*) FROM job_roles');
    const jobRolesCount = parseInt(jobRolesResult.rows[0].count, 10);
    console.log(`Found ${jobRolesCount} job roles in destination database`);
    
    if (capabilitiesCount === 0 || jobRolesCount === 0) {
      console.error('⚠️ Dependencies not fully migrated! Make sure to run the ai-capabilities-migrate.js and job-roles-migrate.js scripts first.');
      return false;
    }
    ...
    jobRolesClient.release();
  }
}

async function main() {
  try {
    console.log("Starting capability-job roles mapping migration...");
    ...
    const beforeCount = await countRows(destPool, 'capability_job_roles');
    console.log(`Current row count in capability_job_roles: ${beforeCount}`);
    
    // Clear the table first
    await clearTable(destPool, 'capability_job_roles');
    
    // Get data from source
    const mappingData = await getTableData(sourcePool, 'capability_job_roles');
    
    // Insert data into destination
    await insertData(destPool, 'capability_job_roles', mappingData);
    
    // Get count after migration
    const afterCount = await countRows(destPool, 'capability_job_roles');
    console.log(`Row count in capability_job_roles after migration: ${afterCount}`);
    
    console.log("Capability-job roles mapping migration completed");
  } catch (error) {
