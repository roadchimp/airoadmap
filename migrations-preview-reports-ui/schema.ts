import { pgTable, foreignKey, serial, integer, text, numeric, boolean, jsonb, timestamp, uniqueIndex, unique, primaryKey, pgMaterializedView, json, pgEnum } from "drizzle-orm/pg-core"
...
export const assessmentResponses = pgTable("assessment_responses", {
	...
	foreignKey({
			...
			name: "assessment_responses_user_id_user_profiles_id_fk"
		}).onDelete("cascade"),
]);
...
export const jobDescriptions = pgTable("job_descriptions", {
	...
	foreignKey({
			...
			name: "job_descriptions_organization_id_organizations_id_fk"
		}),
	foreignKey({
			columns: [table.jobRoleId],
			foreignColumns: [jobRoles.id],
			name: "job_descriptions_job_role_id_job_roles_id_fk"
		}),
]);
...
export const assessments = pgTable("assessments", {
	...
	foreignKey({
			columns: [table.userId],
			foreignColumns: [userProfiles.id],
			name: "assessments_user_id_user_profiles_id_fk"
		}),
]);
...
export const reports = pgTable("reports", {
	...
	foreignKey({
			columns: [table.userId],
			foreignColumns: [userProfiles.id],
			name: "reports_user_id_user_profiles_id_fk"
		}),
]);
...
export const mvDepartmentRoleSummary = pgMaterializedView("mv_department_role_summary", {	departmentId: integer("department_id"),
	departmentName: text("department_name"),
	departmentDescription: text("department_description"),
	roles: json(),
}).as(sql`SELECT d.id AS department_id, d.name AS department_name, d.description AS department_description, COALESCE(json_agg(json_build_object('id', jr.id, 'title', jr.title, 'departmentId', jr.department_id, 'level', jr.level, 'skills', jr.skills, 'description', jr.description, 'keyResponsibilities', jr.key_responsibilities, 'aiPotential', jr.ai_potential, 'is_active', jr.is_active, 'created_at', jr.created_at, 'updated_at', jr.updated_at)) FILTER (WHERE jr.id IS NOT NULL), '[]'::json) AS roles FROM departments d LEFT JOIN job_roles jr ON d.id = jr.department_id GROUP BY d.id, d.name, d.description ORDER BY d.name`);