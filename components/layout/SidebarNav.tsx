  ...
  return (
    ...
    <div className="flex flex-col h-full w-64 bg-white border-r border-gray-200">
      ...
      <nav className="flex-grow overflow-y-auto py-6 px-3 space-y-1">
        ...
        {/* Scoring Settings Link (NEW) */}
        <Link
          href="/settings/scoring" // New Path
          onClick={onClose}
           className={`${baseLinkClasses} ${
            isActive('/settings/scoring') // New Path for isActive check
              ? `${activeLinkClasses} ${activeIconClasses}`
              : `${inactiveLinkClasses} ${inactiveIconClasses}`
          }`}
        >
          {/* Cog Icon for Settings */}
          <svg className={`mr-3 h-5 w-5 ${isActive('/settings/scoring') ? 'text-gray-700' : 'text-gray-400'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
